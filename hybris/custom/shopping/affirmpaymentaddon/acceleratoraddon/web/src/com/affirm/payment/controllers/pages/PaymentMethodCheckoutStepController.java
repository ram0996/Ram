...
public class PaymentMethodCheckoutStepController extends AbstractCheckoutStepController
{
	...
	public String enterStep(final Model model, final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		if (CheckoutPciOptionEnum.HOP.equals(subscriptionPciOption))
		{
			...
			try
			{
				...
				return AffirmpaymentaddonControllerConstants.HostedOrderPostPage;
			}
		...
		return AffirmpaymentaddonControllerConstants.AddPaymentMethodPage;
	}
   ...
   protected void setupAffirm(Model model) {
      if(affirmPaymentFacade.isAffirmPaymentEnabled() && affirmPaymentFacade.isAffirmPaymentEnabledForCurrentCart()) {
         model.addAttribute("affirmAddressForm", new AffirmAddressForm());
         model.addAttribute("affirmAddressFormUrl", "/checkout/affirm/billing");
         AffirmConfigContainerModel configContainer = getCmsSiteService().getCurrentSite().getAffirmConfigContainer();
         model.addAttribute("affirmModalMode",  configContainer.isModalMode());
         model.addAttribute("affirmVcnMode",  configContainer.isVcnMode());
      }
   }
   ...
   protected String getSilentOrderPostPage() {
      //return PAYMENT_PAGE;
      return "addon:/affirmpaymentaddon/checkout/silentOrderPostPage";
   }

   @RequestMapping(value = "/affirm-vcn", method = RequestMethod.GET)
   @RequireHardLogIn
   public String affirmVCNPayment(final Model model, final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
   {
      ...
      final CartData cartData = getCheckoutFacade().getCheckoutCart();
      model.addAttribute(CART_DATA_ATTR, cartData);
      return "addon:/affirmpaymentaddon/checkout/affirmSilentOrderPostPage";
   }
